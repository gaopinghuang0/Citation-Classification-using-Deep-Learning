...
class BatchRNN(nn.Module):
    def __init__(self, embedding_dim, hidden_dim, batch_size,
        ...
        if rnn_model in ["RNN", "LSTM", "GRU"]:
            if rnn_model == "RNN":
                self.rnn_model = nn.RNN(embedding_dim, hidden_dim, dropout=dropout, batch_first=True)
            elif rnn_model == "LSTM":
                self.rnn_model = nn.LSTM(embedding_dim, hidden_dim, dropout=dropout, batch_first=True)
            elif rnn_model == "GRU":
                self.rnn_model = nn.GRU(embedding_dim, hidden_dim, dropout=dropout, batch_first=True)
            else:
                raise ValueError("Speficy rnn_model. RNN, LSTM, or GRU. Default is LSTM.")
    ...
    def init_hidden(self):
        ...
        if isinstance(self.rnn_model, nn.LSTM):
    ...
    def forward(self, sentences, seq_lengths):
        ...
        out, hidden = self.rnn_model(pack, self.hidden)
        ...
        # only use the last time step output of GRU for each sequence
